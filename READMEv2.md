...
* [x] **索引器:** 更新 `RunIncrementalIndexing` 以支持传入**目录切片 (`[]string`)**，并串联解析与存储逻辑。

### Phase 2: 上下文选择 (Context Selection)

* [x] **Agent 集成:** 移植旧版 Agent (`agent`, `models` 包) 到新架构。
* [x] **选择器:** 实现 `context/selector.go`，联通本地 LLM 与 Redis 索引。

### Phase 3: 生成与执行 (Execution)

* [x] **编辑器:** 实现 `editing/ast_editor.go`，完成基于 AST 的精准替换逻辑。
* [x] **工作流:** 实现 `workflow/goal_runner.go`，串联 Context -> Cloud Agent -> Editor 闭环。
